...
# Evaluated by node position classification (NIC) on large graphs
...
def NPC_eva(node_set, emb, gnd, train_ratio=0.2, n_splits=10, random_state=0, C=1.):
    # ====================
    emb_ = emb[node_set]
    gnd_ = gnd[node_set]
    micro, macro = [], []
    ...
    for trn_idx, tst_idx in shuffle.split(emb_):
        ...
        emb_trn, emb_tst = emb_[trn_idx], emb_[tst_idx]
        gnd_trn, gnd_tst = gnd_[trn_idx], gnd_[tst_idx]
...
if __name__ == '__main__':
    ...
    parser.add_argument('--norm', type=int, default=2) # 0: n/a, 1: z-norm, 2: l2-norm
    ...
    for tr in tr_list:
        mi_mean, mi_std, ma_mean, ma_std = \
            NPC_eva(node_set, emb, gnd, train_ratio=tr, n_splits=10, random_state=rand_seed_gbl)
    print()
